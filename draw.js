;window.onload = (event) => {
	...
	data.games.forEach(game => {
		...
		game.results.filter(result => result.score > 0).forEach(result => gamesHtml += '<tr><td>' + data.players.find(player => player.id == result.player).name + '</td><td>+' + result.score + '</td></tr>');
